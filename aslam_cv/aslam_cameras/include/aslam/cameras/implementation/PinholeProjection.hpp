...
bool PinholeProjection<DISTORTION_T>::initializeIntrinsics(const std::vector<GridCalibrationTargetObservation> &observations) {
  ...
  if(f_guesses.empty()) {
    ...
    if(manual_input != nullptr) {
      ...
      double input_guess = std::atof(manual_input);
      ...
      if (input_guess == 0.0){
        ...
        std::cout << "Using environment variable initialization set with KALIBR_MANUAL_FOCAL_LENGTH_INIT : " << input_guess << std::endl;
      }
